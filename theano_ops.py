...
class IntegrateVectorizedGeneralized(theano.Op):
    ...
    '''
    def __init__(self, expr, f, var, bins, *inputs, num_nodes=5):
        super().__init__()
        self.f = f
        self._expr = expr
        self._var = var
    ...
    def L_op(self, inputs, output, output_grads):
        ...
        out, = output_grads
        dargs = []
        for gradient in self.precomputed_grads:
            # from IPython import embed; embed()
...
if __name__ == '__main__':
    ...
    expr = amplitude_ * energy **(-alpha_ - beta_ * T.log10(energy))

    integrator = IntegrateVectorizedGeneralized(expr, f, energy, bins, amplitude_, alpha_, beta_)
